...
# Step 9: Configure Authentication Fix
log_info "Step 9: Setting up authentication fixes..."

# Add bcrypt to backend requirements if not already present
if ! grep -q "bcrypt==" "backend/requirements.txt"; then
    log_info "Adding bcrypt dependency to backend/requirements.txt"
    run_command "grep -q 'passlib==' backend/requirements.txt && sed -i '/passlib==/a bcrypt==4.1.2' backend/requirements.txt" "Adding bcrypt after passlib"
    log_success "Added bcrypt dependency to requirements.txt"
else
    log_info "bcrypt dependency already exists in requirements.txt"
fi

# Ensure JWT token authentication is properly configured
log_info "Configuring JWT token settings..."

# Check if auth-fix.js exists, create if not
if [ ! -f "auth-fix.js" ]; then
    log_info "Creating auth-fix.js script..."
    cat > "auth-fix.js" << 'EOF'
...
EOF
    chmod +x auth-fix.js
    log_success "Created auth-fix.js script"
fi

# Set up PM2 to manage the auth fix service
# Check if PM2 is installed
if command -v pm2 &> /dev/null; then
    log_info "PM2 found, configuring auth-fix service..."
    
    # Create a simple PM2 startup script
    cat > "./start-auth-fix.sh" << 'EOF'
#!/bin/bash
node auth-fix.js
EOF
    ...
    if pm2 list | grep -q "auth-fix"; then
        log_info "Restarting auth-fix service..."
        run_command "pm2 restart auth-fix" "Restarting auth-fix service"
    else
        log_info "Starting auth-fix service for the first time..."
        run_command "pm2 start ./start-auth-fix.sh --name auth-fix" "Starting auth-fix service"
        run_command "pm2 save" "Saving PM2 configuration"
    fi
    
    log_success "Authentication fix service is now running with PM2"
else
    # PM2 not available, use systemd if possible
    log_info "PM2 not installed, checking for systemd..."
    
    if command -v systemctl &> /dev/null; then
        log_info "Setting up systemd service for auth-fix..."
        
        # Create systemd service file
        cat > "/tmp/probeops-auth-fix.service" << EOF
[Unit]
Description=ProbeOps Authentication Fix Service
...
ExecStart=$(which node) $(pwd)/auth-fix.js
Restart=on-failure
RestartSec=10
StandardOutput=syslog
StandardError=syslog
SyslogIdentifier=probeops-auth-fix
...
EOF
        ...
        # Install the service
        run_command "sudo mv /tmp/probeops-auth-fix.service /etc/systemd/system/" "Installing systemd service"
        run_command "sudo systemctl daemon-reload" "Reloading systemd"
        run_command "sudo systemctl enable probeops-auth-fix.service" "Enabling auth-fix service"
        run_command "sudo systemctl restart probeops-auth-fix.service" "Starting auth-fix service"
        
        log_success "Authentication fix service is now running with systemd"
    else
        # Neither PM2 nor systemd available, use nohup
        log_info "Neither PM2 nor systemd available, using nohup..."
        
        # Kill any existing process
        run_command "pkill -f 'node auth-fix.js' || true" "Stopping any existing auth-fix process"
        
        # Start with nohup
        run_command "nohup node auth-fix.js > auth-fix.log 2>&1 &" "Starting auth-fix service with nohup"
        
        log_success "Authentication fix service is now running in the background"
    fi
fi

log_success "Authentication fixes applied successfully"

# Step 10: Handle docker volumes
log_info "Step 10: Setting up Docker volumes..."
